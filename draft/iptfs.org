...
algorithm is the loss rate and the round-trip time (RTT). These
values are provided by IP-TFS using the congestion information
...
the RTT ([[RFC3448]]). To provide the sender with these values the
receiver (egress tunnel endpoint) sends congestion information (CI)
reports using a fixed interval timer.

The congestion information consists of packet drop reports indicating
the number of packet drops that have occurred over a sequence of
packets. The sequence of sent packets is identified by the start and
end ESP sequence numbers.

The loss rate is the number of packet drops divided by the time
interval between sending the start and end packets as indicated by
the provided sequence number range. The time interval is simply the
number of packets in the range multiplied by the current packet send
rate.

# XXX Expand on the following.

The sender can also approximate the packet RTT based on when it
receives the congestion information for the end packet sequence
number contained within said CI report.

These congestion information reports MAY also be sent when in the
non-congestion controlled mode to allow for reporting loss and RTT
from the ingress sender as well as to allow implementing Circuit
Breakers [[RFC8084]].
...
- RcvSeqStart :: A 32 bit unsigned integer containing the first ESP
                 sequence number (as defined in [[RFC4303]]) of the
                 packet range that this information relates to.
- RcvSeqEnd :: A 32 bit unsigned integer containing the last ESP
               sequence number (as defined in [[RFC4303]]) of the
               packet range that this information relates to.
- Timestamp :: A 32 bit unsigned integer containing the lower 32 bits
               of a running monotonic millisecond timer of when the
               RcvSeqEnd packet was received. This value is used to
               determine the packet transit time. Implementations
               should deal with wrapping of this timer value.
