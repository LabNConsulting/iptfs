...
pair. This document defines 3 "TFS Type" values: None (0),
TFS_IPTFS_CC (1) for congestion-controlled IP-TFS mode or
TFS_IPTFS_NOCC (2) for non-congestion controlled IP-TFS mode. The
selection of a proposal with a "TFS Type" value TFS_IPTFS_CC or
TFS_IPTFS_NOCC does not mandate the use of IP-TFS, rather it
indicates a willingness or intent to use IP-TFS on the SA pair. A new
Notify Message Status Type IPTFS_REQUIREMENTS (TBD3) is then used by
the initiator and responder to further refine any operational
requirements.

** IPTFS_REQUIREMENTS Status Notification

As mentioned in the previous section, a new Notify Message Status
Type IPTFS_REQUIREMENTS (TBD3) is used by the initiator and responder
to further refine what will be supported. This notification is sent
during IKE_AUTH and new CREATE_CHILD_SA exchanges; however, it MUST
NOT be sent, and MUST be ignored, during a CREATE_CHILD_SA rekeying
exchange as the requirements are not allowed to change during
rekeying.
...
message. The flag values (currently a single flag) are defined below.
The initiator and responder MAY both include this notification. If
the IPTFS_REQUIREMENTS notification is not sent then it implies that
all the flag bits are clear.
...
- 0 :: Set to zero on send and checked for zero on receive.
- D :: Don't Fragment bit, if set indicates the sender of the notify
       message does not support receiving packet fragments (i.e.,
       inner packets MUST be sent using a single Datablock). This
       value only applies to what the sender is capable of receiving;
       the sender MAY still send packet fragments unless similarly
       restricted by the receiver in it's IPTFS_REQUIREMENTS
       notification.

If any unknown IPTFS_REQUIREMENTS flag bit are set, a receiving
responder should not select any IPTFS proposal, and a receiving
initiator should fail the child SA creation if an IPTFS proposal was
selected by the responder. This is done to allow for adding future
requirements in a backward compatible way. In both the responder and
the initiator case the receiver should send a Notify Message with
Error Type IPTFS_UNKNOWN_REQUIREMENTS (TBD4) back to the sender, and
include with this notification an octet with the unknown bits set.
For the responder this would be sent in the IKE_AUTH or
CREATE_CHILD_SA response, and for the initiator it would be included
in a new INFORMATIONAL exchange. This allows the sender of the
IPTFS_REQUIREMENTS message to determine which requirements are not
...
** IPTFS_UNKNOWN_REQUIREMENTS Notify Message Error Type

This document requests a status type IPTFS_UNKNOWN_REQUIREMENTS be
allocated from the "IKEv2 Notify Message Types - Error Types"
registry.

  - Value :: TBD4
  - Name :: IPTFS_UNKNOWN_REQUIREMENTS
  - Reference :: This document
