...
The egress (receiving) of the IP-TFS tunnel MUST allow for and expect
the ingress (sending) side of the IP-TFS tunnel to vary the size and
rate of sent encapsulating packets, unless constrained by other
policy.
...
the DS field SHOULD NOT be copied although a sender MAY
choose to allow for configuration to override this behavior. A
sender SHOULD also allow the DS value to be set by
configuration.

It is worth noting that a sender MAY still set the ECN value of
...
enabled SA. In other words, an SA that has
...
MUST NOT update the recorded time. When the receiver sends its CC
...
on the tunnel (i.e., the average time between sending packets on its
half of the IP-TFS tunnel). When the sender receives back its ~TVal~
in the ~TEcho~ header field it calculates 2 RTT estimates. The first
is the actual delay found by subtracting the ~TEcho~ value from its
...
between sending packets on its half of the IP-TFS tunnel). The
...
specify requirements from the sender of the message. If any
...
AGGFRAG_PAYLOAD which has the value 0x5. The first octet of this
