...
SHOULD be configurable by the user. Finally, we note that as IP-TFS
...
be processed as they leave the window.

**** Optional Extra Padding

It has been suggested that when an IP-TFS tunnel's bandwidth is not
being fully utilized, an implementation could use an algorithm which
would delay sending inner packets some amount of time in order to
avoid fragmentation (i.e., pad-out the current encapsulating packet
in order to delivery the inner packet un-fragmented in the next outer
