...
When an inner packet is fragmented across multiple IP-TFS payloads
the inner packet fragments MUST be sent in order. The fragments MUST
also be sent using consecutive sequence numbers with one exception.
Within an inner packet fragment sequence the sender is allowed to
insert "all-pad" IP-TFS payloads (i.e., payloads with a ~BlockOffset~
of zero and a single pad ~DataBlock~). This all-pad exception allows
for the insertion of "all-pad" payloads within the inner packet
fragment stream to maintain correct timing regardless of any
encapsulation computational requirements. When a receiver is
reassembling an inner packet and it receives an "all-pad" payload it
simply increments the next fragment expected sequence number.
...
this, these mappings mappings are restricted further. In particular
...
operate using IP-TFS at startup, IP-TFS operation is allowed to be
enabled post-SA creation while in use. This late-enabling may be
useful for debugging or other purposes. To support this late-enabled
