...
obscuring the size and frequency of IP traffic using a constant
sized, constant send rate IPsec tunnel. The solution allows for
...
accommodated. While this document does not standardize the congestion
control algorithm, the algorithm used by an implementation SHOULD
conform to the guidelines in [[RFC2914]].
...
tunnel endpoint) MUST send regular packet drop reports to the
sender (ingress tunnel endpoint). These reports indicate the number
of packet drops during a sequence of packets. The sequence or range
of packets is identified using the start and end ESP sequence numbers
of the packet range.
