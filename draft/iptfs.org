...
at which that ingress sends packets. While this document does not
require a specific congestion control algorithm, best current
practice RECOMMENDS that the algorithm conform to [[RFC3448]]. Congestion
...
algorithm is the loss rate and an estimated round-trip time (RTT).
These values are provided by IP-TFS using the congestion information
reports described in [[Congestion Information]]. In particular these
values are sufficient to implement [[RFC4342]].
...
general, needs to know the loss rate and also be able to approximate
the RTT ([[RFC3448]]). In order to obtain these values the sender
periodically requests congestion information from the receiver using
an IKEv2 information exchange [[RFC7296]].
...
- ResvFlags :: 7 bits set to 0.

***  IKEv2 Information Request Congestion Information Payload (TBD).

The following defines the information exchange payload type
IPTFS_CONGEST_INFO used in the IKEv2 [[RFC7296]] to request a congestion
information report. This is sent by the sender to the receiver to
request a new congestion information report.

The first 4 octets are the standard generic payload header fields
defined in [[RFC7296]]. The IPTFS_CONGEST_INFO Payload Type value is
TBD.
...
- Payload Length :: 2 octet length set to 5.
- ResendCount :: An 7 bit value, set to 1 on the initial sending of
                 this information request, and incremented by 1 for
                 each time the request is retransmitted (i.e., b/c it
                 has not be responded to yet).

***  IKEv2 Information Response Congestion Information Payload

When the receiver receives an IKEv2 information exchange request with
the IPTFS_CONGEST_INFO payload it replies with the following data in
response.
...
- ResendCount :: A 7 bit field containing the value from the
                information request field of the same name for which
                this data is a response to.
- DropCount :: A 24 bit unsigned integer count of the drops that
               occurred between RcvSeqStart and RcvSeqEnd. If the
               drops exceed the resolution of the counter then set to
               the maximum value (i.e., 0xFFFFFF).
- RcvSeqStart :: A 64 bit unsigned integer containing the first ESP
                 sequence number (as defined in [[RFC4303]]) of the
                 packet range that this information relates to.
- RcvSeqEnd :: A 64 bit unsigned integer containing the last ESP
               sequence number (as defined in [[RFC4303]]) of the
               packet range that this information relates to.
- OverheadTime :: A 32 bit unsigned integer containing an estimate in
                  microseconds of the time between the receipt of the
                  IKEv2 information request, and the sending of this
                  response.
...
Additionally this document requests an IKEv2 payload type
TFS_CONGEST_INFO (TBD) be allocated by IANA from "IKEv2 Payload
Types" registry.

  - Type :: TBD
  - Description :: The IKEv2 information exchange payload for
                   requesting and delivering congestion control
                   information.
  - Reference :: This document
