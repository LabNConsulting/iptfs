...
[[RFC4303]]) type which is identified by the number IPTFS_PROTOCOL
([[IP-TFS Payload]]).
...
An IP-TFS payload is identified by the ESP payload type IPTFS_PROTOCOL
...
The congestion control IPTFS_PROTOCOL payload is comprised of a 24
octet header followed by a variable amount of ~DataBlocks~ data as
shown below.
...
- TVal :: An opaque 32 bit value that will be echoed back by the
          ...
          ~Delay~ value of how long that echo took.
