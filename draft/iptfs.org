...
** A Overhead Comparison of IPSec vs. IP-TFS

The overhead of TFS is 40 bytes per encap packet. Therefor the
octet overhead per inner packet is 40 divided by the number of inner
packets (fractional allowed).

The overhead per inner packet for constant send rate padded IPSec
(i.e., IPSec TFC) is 36 octets plus any padding. So 36 plus the
difference between the remaining MTU and the inner packet
MTU, unless fragmentation is required.

When fragmentation of IPsec is required the overhead is number of
fragments times IP overhead (20) plus a single ESP overhead (16) plus
any required tail padding. The required tail padding is the number of
fragments times the outer MTU times minus the ESP overhead minus the
inner MTU length. So:

     nf*ipoh + espoh + nf*omtu - espoh - imtu
     nf*ipo + nf*omtu - imtu
     nf*(ipo + omtu) - imtu

#+TBLNAME:pohead
#+CAPTION: Overhead of tunnel type in octets
| Type  | overhead |
|-------+----------|
| IP    |       20 |
| IPSec |       36 |
...
#+CONSTANTS: ipo=20 ipso=36 tfso=40
#+CAPTION: Percentage of overhead for TFC contstant send rate.
#+TBLNAME:obytes
| Type | IPSec | IPSec | IPSec |    TFS |    TFS |   TFS |
|  MTU |   576 |  1500 |  9000 |    576 |   1500 |  9000 |
|------+-------+-------+-------+--------+--------+-------|
|   64 |   512 |  1436 |  8936 |   4.78 |   1.75 |  0.29 |
|  536 |    40 |   964 |  8464 |  40.00 |  14.68 |  2.39 |
| 1460 |   328 |    40 |  7540 | 108.96 |  40.00 |  6.52 |
| 1500 |   288 |  1540 |  7500 | 111.94 |  41.10 |  6.70 |
| 8960 |  1172 |  1680 |    40 | 668.66 | 245.48 | 40.00 |
| 9000 |  1132 |  1640 |  9040 | 671.64 | 246.58 | 40.18 |
|      |       |       |       |        |        |       |
 #+TBLFM: @3$2..@8$4=if(@2-$ipso > $1, $ipso+(@2-$ipso-$1), ceil($1/(@2-$ipo)) * ($ipo + @2) - $1)::@3$5..@8$7=$tfso/((@2-$tfso)/$1);%.2f

#+CAPTION: Percentage of overhead for TFC contstant send rate.
#+TBLNAME:avail-pct
| Type | IPSec 576 | IPSec 1500 | IPSec 9000 | TFS 576 | TFS 1500 | TFS 9000 |
|      |           |            |            |         |          |          |
|------+-----------+------------+------------+---------+----------+----------|
|   64 |    800.0% |    2243.8% |   13962.5% |    7.5% |     2.7% |     0.5% |
|  536 |      7.5% |     179.9% |    1579.1% |    7.5% |     2.7% |     0.4% |
| 1460 |     22.5% |       2.7% |     516.4% |    7.5% |     2.7% |     0.4% |
| 1500 |     19.2% |     102.7% |     500.0% |    7.5% |     2.7% |     0.4% |
| 8960 |     13.1% |      18.8% |       0.4% |    7.5% |     2.7% |     0.4% |
| 9000 |     12.6% |      18.2% |     100.4% |    7.5% |     2.7% |     0.4% |
#+TBLFM: $1=remote(obytes,@@#$1)::@3$2..@8$7=100*remote(obytes,@@#$$#)/$1;%.1f%%

#+begin_src gnuplot :var data=avail-pct :file auto/tfs-ipsec-overhead.png
  set title "IPSec vs TFS Overhead"

  red = "#AA0000"; green = "#00AA00"; blue = "#0000AA";
  lightred = "#FF0000"; lightgreen = "#00FF00"; lightblue = "#0000FF";
  set yrange [0:1000]
  ...
  plot data using 2 title "IPSec 576" linecolor rgb red, '' using 3 title "IPSec 1500" linecolor rgb blue, '' using 4 title "IPSec 1500"  linecolor rgb green, '' using 5 title "IPSec 576" linecolor rgb lightred, '' using 6 title "IPSec 1500" linecolor rgb lightblue, '' using 4 title "IPSec 1500"  linecolor rgb lightgreen
#+end_src

#+RESULTS:
[[file:auto/tfs-ipsec-overhead.png]]
