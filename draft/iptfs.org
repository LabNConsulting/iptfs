...
While normally a user will configure their IPsec tunnel (SA) to
...
At minimum the congestion information must be sent, from the receiver
...
See [[A Send and Loss Event Rate]] for a suggested way to calculate
...
round trip time estimate (provided by the sender) and ~p~ is the loss
event rate.
